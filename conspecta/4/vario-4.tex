...
\ok
    ...
    Значит, $(F'(x_0) - \lambda G'(x_0))(h) = (const)(F'(x_0) \eta - \lambda G'(x_0)\eta)$.
    ...
    Пусть $u_0$ --- точка локального экстремума $J\big|_{\bigcap \Ker R_j}$; оператор $\R(u_0) \coloneqq \vect{R_1'(u_0) \\ \vdots \\ R_n'(u_0)}$ имеет полный ранг.
...
% Вот если бы вы выбрали курс по потенциалам, \textbf{А вы не выбрали!}, то там бы вам рассказали об этом подробнее
    ...
    \item $\Fc(z, w) = |w|\cdot f(z)$, где, например, $n = 2$ (плоскость), $f(z) = z_2^{\alpha}$ (здесь $z = \vect{z_1\\z_2}$).
    \bullets{
    ...
    \item При $\alpha = -\frac{1}{2}$ это время, требуемое шариком, чтобы скатиться по жёлобу данной формы.
    }
    }
...
%    Сформулируем некоторое техническое утверждение, которым будем пользоваться в меньшей общности, и докажем не здесь, но сформулируем сейчас
    ...
    $J = \int \Fc[\gamma, \dot{\gamma}]\d t$, $E\{\gamma\} = (\nabla_{z} \Fc)(\gamma, \dot{\gamma}) - \frac{\d}{\d t}\nabla_w \Fc(\gamma, \dot{\gamma})$ --- определение осмысленно, так как кривая параметризована, и $\dot{\gamma} \ne 0$, а $\Fc \in C^2(\dots)$.
    ...
    \theorem{
    Пусть $\gamma \in \Gamma^2$ --- локальный максимум $J$, $\Fc \in C^2(\R^n \times (\R^n \sm \{0\})), \Fc(z, \lambda w) = \lambda \Fc(z, w), \lambda > 0$.
    ...
    Рассматриваем функционал на пространстве $X = \defset{\gamma \in \Gamma^2}{\gamma(a_\gamma) \in M_1, \gamma(b_\gamma)\in M_2}$.
    ...
    Далее считаем, что всё перепараметризовано так, что все кривые определены на $[0, 1]$, \comment{так что ли}. %Да, вполне
    ...
    Также считаем, что $\|s(\_, \tau)\|$ \comment{что?} %Тут, видимо, как раз \|\der{s}{t}\| отделена от нуля
    ...
    $\nabla_w\Fc = \frac{w}{|w|}\then\frac{\d}{\d t}(\frac{\dot{\gamma}_0}{|\dot{\gamma}_0|}) = 0$ (уравнение Э-Л).
    ...
    $\nabla_{w}\Fc = g(z)\frac{w}{|w|}$. В этом случае условия трансверсальности тоже сводятся к условиям ортогональности $\dot{\gamma_0}(a) \perp M_1, \dot{\gamma_0}(b) \perp M_2$.
    }
    ...
    \[\all{L + \der{L}{\dot{y}}(\dot{\phi}(a) - \dot{y}(a)) = 0 \\ L + \der{L}{\dot{y}}(\dot{\psi}(b) - \dot{y}(b)) = 0}\] % если захочешь вернуть аргументы у L, то тогда напиши и у \der{L}{\dot y}
%    Теперь --- фокусы
...
%
%    \end{tikzpicture}
    \multline{J[\tilde{y}] - J[y] = \int\limits_{x_0 + \delta x_0}^{x_1 + \delta x_1}L(x, \tilde{y}, \dot{\tilde{y}}) - \int\limits_{x_0}^{x_1}L(x, y, \dot{y}) = \int\limits_{x_0}^{x_1}(L(\tilde\dots) - L(\dots)) - \int\limits_{x_0}^{x_0 + \delta x_0}L(\tilde\dots)  + \int\limits_{x_1}^{x_1 + \delta x_1}L(\tilde\dots) = \\=
    \int\limits_{x_0}^{x_1}\left(\der{L}{y} - \frac{\d}{\d x}\der{L}{\dot{y}}\right)h \d x + L(x_1, y(x_1), \dot{y}(x_1))\delta x_1 - L(x_0, y(x_0), \dot{y}(x_0))\delta x_0 +\\+ \bigO(\|h\|^2_{C^1}) + o(\delta x_1) + o(\delta x_2) + \der{L}{\dot y}h\big|_{x_0}^{x_1}\circlesign{=}}
    ...
    Пусть $T: \R^n \map \R^n$ --- диффеоморфизм, $J[\gamma] = \int\Fc(\gamma, \dot{\gamma})\d t, \Fc(z, \lambda w) = \lambda\Fc(z, w), \lambda > 0$ \comment{(это и раньше требовалось, надо дописать)}. % что именно дописать? \lambda > 0?
    ...
    Согласно когда-то проделанной выкладке: $\delta J_T[\gamma, h] = \int\angles{E_T\{\gamma\}, h}\d t$. % не особо искал, но о-малое по идее должно быть
    \multline{
        J_T[\gamma + h] - J_T[\gamma] = J[T(\gamma) + T'(\gamma)h + r] - J_T[\gamma] = \int\left[\angles{\der{\Fc}{\gamma}, T'(\gamma)h + r} + \angles{\der{\Fc}{\dot{\gamma}}, (T'(\gamma)h + r)'}\right]\d t + o(\|h\|_{C^1}) =\\=
    ...
    Рассмотрим задачу с фиксированными концами для функционала $J[u] = \int\limits_{a}^{b}(p u'^2 + qu^2)\d x$.
    ...
    Задача поиска решения уравнения~\eqref{sturm} при условиях $\all{\alpha_a u(a) + \beta_a u'(a) = 0 \\ \alpha_b u(b) + \beta_b u'(b) = 0} \left(\text{считается, что }\all{\alpha^2_u + \beta^2_a \ne 0\\ \alpha^2_b + \beta^2_b \ne 0}\right)$ называется задачей Штурма --- Лиувилля.
    ...
    Отсюда сразу следует равностепенная непрерывность: \[|u_n(x) - u_n(x')| = \abs{\int\limits_{x}^{x'}u'_n} \le \sqrt{|x - x'|} \cdot \left(\int\limits_{x}^{x'}u_n'^2\right)^{\nicefrac{1}{2}}\]
    ...
    Если бы можно было проинтегрировать по частям ($u \in C^2$ и удовлетворяет уравнению), то мы, конечно же, это очень просто получили. Но мы проверим это иначе:

    Попробуем поварьировать $J$.
    ...
    Интегрируя по частям линейный по $\eps$ член, получаем $2\eps\int\limits_{a}^{b}u_n (-(ph')' + qh)$, внеинтегральные члены обнулились.
    ...
    Выберем $h(x) \coloneqq -\int\limits_{a}^{x}\frac{1}{p(t)} \left(\int\limits_{a}^{t}\xi(s)\d s + C\right)\d t$, где $C$ выбрана так, что $h(b) = 0$.
    А именно, $$C = - \frac{\int\limits_{a}^{b}\frac{\d t}{p(t)}\int\limits_{a}^{t}\xi(s)\d s} {\int\limits_{a}^{b}\frac{\d t}{p(t)}}$$

    По построению $h \in C^2, h(a) = h(b) = 0$.
    \multline{
        0 = \int\limits_{a}^{b}\xi u - \int\limits_{a}^{b}(q(x) - \lambda_J)u(x)\int\limits_{a}^{x}\frac{1}{p(t)}\left(\int\limits_{a}^{t}\xi(s)\d s + C\right)\d t = \int\limits_{a}^{b}\xi u - \int\limits_{a}^{b}\xi(s)\d s \int\limits_{s}^{b}\frac{\d t}{p(t)}\int\limits_{t}^{b}(q(x) - \lambda_J)u(x)\d x +\\+ \frac{1}{\int\limits_{a}^{b}\frac{1}{p}}\int\limits_{a}^{b}\xi(s)\int\limits_{s}^{b}\frac{\d t}{p(t)}\int\limits_{a}^{b}(q(x) - \lambda)u(x)\int\limits_{a}^{x}\frac{1}{p}
    }

    Обзовём $K \coloneq \left({\int\limits_a^b \frac 1p}\right)^{-1}\int\limits_{a}^{b}(q(x) - \lambda)u(x)\int\limits_{a}^{x}\frac{1}{p}$ (константа, не зависящая от $\xi$), получаем
    
    $$0 = \int\limits_a^b \xi(s)\left[u(s) - \int\limits_s^b \frac{\d t}{p(t)}\int\limits_t^b(q(x) - \lambda_J)\d x + K \int\limits_{s}^b \frac{\d t}{p(t)}\right]\d s$$
    
    Итак, $0 = \int\limits_{a}^{b}\xi(s)\left[\cdots\right]$, откуда по лемме Дюбуа-Реймона выражение в скобках равно нулю везде, $u \in C^1$.
    

    Значит, $u$ можно продифференцировать, получим $$u'(s) = -\frac1{p(s)}\int\limits_s^b(q - \lambda_J)u - \frac{K}{p(s)},$$ откуда $u \in C^2$.
    Тем самым, $(p(s) u'(s))' = (q(s) - \lambda_J)u(s)$, значит, $u \in X$, $J[u] = \int\limits_{a}^{b}p u'^2 + qu^2$.
        Интегрируя по частям, получаем ровно $\lambda_J \int\limits_{a}^{b}u^2 = \lambda_J$.
    }
    }
    \corollary{
    ...
    Сами $\lambda$, для которых~\eqref{sturm} имеет решение, называются \emph{собственными числами}, и соответствующие функции $u$ --- \emph{собственные функции}.
    ...
    Предположим, что $\lambda_n \nearrow \lambda_* < \infty$.
        ...
        Но, расписав $J$, получаем $\int\limits_{a}^{b}pu_n'^2 + q u_n^2 \ge \min q + \min p \int\limits_{a}^{b}u_n'^2$, получаем, что в последовательности $u_n$ функции равномерно ограничены и равностепенно непрерывны, а значит, имеется подпоследовательность $u_{n_k}$, сходящаяся в $C[a, b]$ к $u_{@}$. Это противоречие к попарной ортогональности:
    ...
    \[J\left(\frac{f - f_N}{\|f - f_N\|}\right) \ge \lambda_{N+1} \iff \underbrace{J[f - f_{N}]}_{\int\limits_{a}^{b}p(f' - f'_N)^2 + q(f - f_N)^2} \ge \lambda_{N+1}\|f - f_{N}\|^2\]
    ...
    \corollary{
        \(\{\lambda_j\}_{j = 1}^{\infty} = \defset{\lambda \in \C}{\text{\eqref{sturm} имеет решение для }u \in C^2\text{ при условии }u(a) = u(b) = 0, u \not\equiv 0}\)
    \provehere{ $\subseteq$ уже доказали выше. Обратное включение докажем от противного.
        ...
        \[\lambda_*(u_*, u_n)_{L^2} = (\mathscr{L}u_*, u_n)_{L^2} = \int(-(pu_*')' + qu_*) u_n = \int u_*((-pu_n')' + qu_n) = \lambda_n(u_*, u_n)_{L^2}\] \[\then \forall n\ (u_*, u_n) = 0 \then u_* = 0\]
    ...
    \theorem{
        ...
        \provehere{
            ...
            $u(x_*, \lambda_*) = 0$.
            Рассмотрим $u(x_* - \eps, \lambda)$ и $u(x_* + \eps, \lambda_*)$.
