...
%    \end{tikzpicture}
    ...
    Попробуем поварьировать $J$.
        ...
        \[\tilde{J}[u_n + \eps h] = J[u_n] + 2\eps\int\limits_{a}^{b}(pu_n h h' + qu_n h) + \eps^2 \tilde{J}[h]\]
    Интегрируя по частям линейный по $\eps$ член, получаем $-2\eps\int\limits_{a}^{b}u_n (-(ph')' + qh)$, внеинтегральные члены обнулились.

    С другой стороны, $J[u_n + \eps h] \ge \lambda_J \int\limits_{a}^{b}(u_n + \eps h)^2 = \lambda_J + 2\eps\lambda_J \int\limits_{a}^{b} u_n h + \eps^2 \lambda_J \int\limits_{a}^{b}h^2$.
