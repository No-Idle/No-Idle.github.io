...
\begin{document}
    ...
    Поскольку $R(a) = 0$, интегируя по частям, получим $\angles{R(b), h(b)} + \int\limits_{a}^{b}\Bigl\langle\underbrace{(\nabla_{\dot{u}}L)(t, u(t), \dot{u}(t)) - R(t)}_{\xi(t)}, \dot{h}(t)\Bigr\rangle\d t$
...
\ok
    ...
    \item Определеённое в предыдущем пункте отображение $G': X \map X^*$ непрерывно.
    }
...
%    Теперь --- фокусы
    ...
    \multline{
        J_T[\gamma + h] - J_T[\gamma] = J[T(\gamma) + T'(\gamma)h + \gamma] - J[T(\gamma)] =\\= \int\left[\angles{\der{\Fc}{\gamma}, T'(\gamma)h + r} + \angles{\der{\Fc}{\dot{\gamma}}, (T'(\gamma)h + r)'}\right]\d t + o(\|h\|_{C^1}) =\\=
