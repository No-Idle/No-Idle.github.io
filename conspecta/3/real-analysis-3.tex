...
\begin{document}
    ...
    \note{
        Можно аддитивно определить длину на всех подмножествах $[a, b]$, но такое определение не конструктивно, и к тому же не единственно.
        Дальше мы определим \emph{меру Лебега}, которая позволяет измерять многие, но всё же далеко не все множества --- можно построить контрпримеры с помощью аксиомы выбора.
    }
    ...
    \section{Меры}
    Пусть $X$ --- множество, $\mathcal{A} \subset 2^X$ --- система его подмножеств.
    ...
    \numbers{
        ...
        \item $\forall a \in \mathcal{A}: \mu(a) = \sup\defset{\mu(K)}{K \subset a;~K \text{ компактно};~K \in \mathcal{A}}$.
    }
    }
    ...
    \examples{
        ...
        Все Борелевские множества, разумеется, измеримы по Лебегу~(\cref{borel-sets}), но обратное неверно --- измеримых множеств сильно больше~(\cref{lebesgue-sets-are-ubiquitous}).
        \item
    ...
    \proposal{\label{lebesgue-sets-are-ubiquitous}
...
%    }
    ...
    \fact{
        Также для последовательности Коши по мере $f_n$ найдётся измеримая $f$, такая, что $f_n \underset{n \to \infty}{\overset{\mu}\Map} f$.
        \provehere{
    ...
    \counterexamples{
    \item Последовательность функций $f_n \coloneqq \chi_{[n, n + 1]}$ сходится почти всюду к $0$, но сходимости по мере нет (\cref{connection} не работает, так как мера бесконечна).
    \item Пусть $H_n \coloneqq \sum\limits_{k = 1}^{n}\frac{1}{k}$ --- гармоническое число.
    ...
    \theorem{
        ...
        \provehere{
            ...
            \[(f * g_j)(x) - g(x) = \int\limits_{\R^n}f(x - y)g_j(y)\d y - f(x) = \int\limits_{\R^n}\left(f(x - y) - f(x) \right)g_j(y)\d y\]
            \multline{\abs{(f * g_j)(x) - g(x) } \le\\\le \int\limits_{|y| < \delta}|f(x - y) - f(x)|g_j(y)\d y + \int\limits_{|y| \ge \delta}\left(|f(x - y)| + |f(x)|\right)g_j(y)\d y \le\\\le \eps + 2K\int\limits_{|y| \ge \delta}g_y(y)\d y \underset{j \to \infty}\rightrightarrows 0}
        }
    }
    \fact{
    ...
    \lemma[Об искажении]{
        Пусть $H: U \map \R^n$, выберем $x_0 \in U$, пусть $H$ непрерывно дифференцируемо и $\d_H(x_0, \_) = E$, тождественный оператор.
        ...
        \provehere{
            Выберем маленький $u \in \R^n$. По определению дифференцируемости $H(x_0 + u) = y_0 + \underbrace{\d_H(x_0, u)}_{u} + v(u)$, где $v(u) = o(|u|)$.
            ...
            Для доказательства левого включения возьмём $H^{-1}$, определённое в некоторой окрестности $y_0$, причём $\d_{H^{-1}}(y_0, \_) = E$.
            ...
            \indent{
                \theorem[Регулярность меры Лебега]{
                    ...
                    \provebullets{
                        ...
                        Можно считать, что они открыты: параллелепипед с номером $j$ можно раздуть так, что $\tilde{P}_j \supset P_j$ открыт, и $\bigl|\tilde{P}_j\bigr| < \frac{\eps}{2^j} + |P_j|$.
                        Положим $U \coloneqq \bigcup\limits_{j = 1}^{\infty}\tilde{P}_j$, тогда $U$ открыто, и $|U| < |e| + 2\eps$.

                        Теперь если $|e| = \infty$, то воспользуемся $\sigma$-конечностью: пусть $\R^k = \bigcup\limits_{s = 1}^{\infty}B_s$, тогда подберём открытые $U_s \supset (e \cap B_s): |U_s \sm (e \cap B_s)| < \frac{\eps}{2^s}$. Объединение $U \coloneqq \bigcup\limits_{s}U_s$ подходит: $U \supset e$, и $|U \sm e| < \eps$.
...
%        Тогда из локальной липшицевости $\mu_m^*(F(e)) = 0$.
            ...
            \item Осталось показать, что $g(x) = |\det \Gamma(\d_f(x, \_))|^{\nicefrac12}$, а это делается с помощью слабой теоремы о дифференцировании.
