...
\begin{document}
    ...
    \precaution{
        Для полукольца возможно бесконечных параллелепипедов теорема Александрова не применима: $\R \times \{0\} \subset \R^2$ не регулярно сверху, всякий параллелепипед, содержащий $\R \times \{0\}$ уже имеет бесконечную меру \comment{(но ведь можно приблизить сколь угодно близко открытым множеством, в чём проблема?)}.
    }
...
%    Если же $\overline{\mu}(E) = \infty$
    ...
    \theorem[Малая теорема Леви]{
        ...
        \provehere{
           ...
           Так как $g$ простая, то $g = \sum\limits_{s = 1}^{n}c_s\chi_{e_s}$, где $e_s$ --- попарно дизъюнктные множества из $\Sigma$.
            ...
            Тем самым, утверждение свелось к следующему: для $e \in \Sigma$, для последовательности простых функций $g_j$, таких, что поточечно $0 \le g_j \le g_{j + 1} \le c\chi_e = g$, причём $\forall x \in X: \lim\limits_{j \to \infty}g_j(x) = c\chi_e(x)$,
    ...
    \theorem{
        ...
        \provehere{
            ...
            Для любого $x \in G$: $\exists c_x \in \mathcal{A}_Y: x \in c_x \subset a$, где $a \in \mathcal{D}$.
    ...
    Таким образом, проверку того, что $f$ измерима, можно выполнять только на Лебеговых множествах.
    \ok
    ...
    \fact{
        ...
        \provehere{
            ...
            \[\int\limits_{X}|f|\cdot|g|\d\mu \le \int\limits_{X}|f(x)|\d\mu\cdot\|g\|_{L^{\infty}(\mu)} = \|f\|_{L^{1}(\mu)}\cdot\|g\|_{L^{\infty}(\mu)}\]
        }
    }
    \note{
    ...
    \comment{Надо проверить, что всё сходится, тут было немного не это написано.}
        \[\|f * g - f\|^p_{L^p} \le \int\limits_{\R^n}\int\limits_{\R^n}|f(x - y) - f(x)|^p\d y\cdot|g(x)| \d x\]
        \provehere{
    ...
    \fact{
    Если $\dist(e_1, e_2) > 0$, то $\mu^*_p(e_1\cup e_2) = \mu_p^*(e_1) + \mu_p^*(e_2)$.
    \provehere{
