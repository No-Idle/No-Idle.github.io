...
%    Если же $\overline{\mu}(E) = \infty$
    ...
    \provewthen{
        ...
        \indentlemma[Неравенство Чебышёва]{
            ...
            Пусть $e = \defset{x}{h(x)>\lambda}$. Заметим, что $h \ge \lambda \chi_e$, из монотонности интеграла $J(h) \ge \lambda \mu(e)$.
        }
    ...
    Тогда во всяком случае $g \ge f_+, h \ge f_-$: \[f = g - h \then f \le g\text{, а так как }g \ge 0\text{, то }f_+ \le g\text{ тоже}\qquad f_- = (-f)_+\]
    \proposal{
    ...
    \note{
        ...
        \provehere{
            Применить неравенство Чебышёва. $\defset{x}{f(x) \ne 0} = \bigcup\limits_{n \ge 0}\bigdefset{x}{|f|(x) \ge \frac{1}{n}}$.
        }
    }
    ...
    \theorem[Вариант теоремы Леви для рядов]{
        Пусть $u_n$ --- неотрицательные суммируемые функции, $\sum\limits_{n = 1}^{\infty}u_n(x) \eqqcolon u(x)$.
    ...
    Осталось применить принцип двух полицейских для проинтегрированного неравенства справа от $\then$ \[0 \le |f_k(x) - f(x)| \le h_k(x) \quad \then \quad \int\limits_{X}0\d\mu \le \int\limits_{X}|f_k(x) - f(x)|\d\mu \le \int\limits_{X}h_k(x)\d\mu\qedhere\]
    }
    }
    \counterexample{
    ...
    \item Если $\mu(X) < \infty$, то из сходимости по мере $f_n \underset{n \to \infty}{\overset{\mu}\Map} f$ следует, что найдётся сходящаяся подпоследовательность: $\exists n_1 < n_2 < \dots$, таких, что $f_{n_k} \almost{k \to \infty} f$.
    }
    \provehere{\down\numbers{
    ...
    \corollary{
    Пусть $\alpha = \inf\defset{p > 0}{\mu_*^p(a) < \infty}$. Тогда $\mu_s^*(a) = 0$ при $s > \alpha$, и $\mu^*_s(a) = \infty$ при $s < \alpha$.
    }
