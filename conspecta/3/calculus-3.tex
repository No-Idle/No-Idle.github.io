...
\begin{document}
    ...
    \example[Полукольцо]{
        Отрезки и конечные отрезки (или даже параллелепипеды~\cref{multiplication_of_semirings}) $\mathcal{P}(\R^n)$ и $\mathcal{P}_0(\R^n)$.
    }
    ...
    \theorem{
        ...
        Утверждается, что $\gamma$ аддитивна~(\cref{why}).
    }
    ...
    \theorem[А. Д. Александров]{
        ...
        \provehere{
            ...
            \bullets{
                \item $\forall k \in \N: \sum\limits_{j =1}^{k} \le \mu(a)$~(\cref{subunion_is_less_than_the_whole}), производим предельный переход.
    ...
    \theorem[Лебег-Каратеодори]{
        ...
        \provebullets{
            ...
            \item Наконец, для доказательства второго дополнения сошлёмся на пятый пункт свойств меры~(\cref{outer-measure}).
        }
    }
    ...
    \examples{
        ...
        Все Борелевские множества, разумеется, измеримы по Лебегу~(\cref{borel-sets}), но обратное неверно.
        \item
...
%    Если же $\overline{\mu}(E) = \infty$
    ...
    \theorem{
        ...
        \provehere{
            ...
            Если же $\dist(e, G_1^\comp) = 0$, то воспользуемся теоремой об исчерпывающей последовательности компактов~\cref{ob-ischerpivauschey}. Найдутся компактные $K_i \subset G$, в объединении дающие $G$.
