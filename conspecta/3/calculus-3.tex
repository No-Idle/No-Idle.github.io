...
%    }
    ...
    Например, ранее записанное равенство $\int\limits_{X}f(x - y)\d y = \int\limits_{X}f(\xi)\d\xi$ видно из данной формулы при $F(y) = x - y$ --- здесь образ меры будет ей самой.
    ...
    \theorem{
        ...
        \provehere{
            ...
            Тем самым, $\|f * g\|^p_{L^p} \le \|g\|_{L^1}^{\frac{p}{q} + 1} \|f\|_{L^p}$ и $\|f * g\|^p_{L^p} \le \|f\|_{L^p}\|g\|_{L^1}$.
        }
    }
    ...
    \fact{
        ...
        \[\|f * g - f\|^p_{L^p} \le \int\limits_{\R^n}\int\limits_{\R^n}|f(x - y) - f(x)|^p\d y \d x\]
        \provehere{
            ...
            \gather{\int\limits_{\R^n}\left(|f(x - y) - f(x)| \cdot 1\right)\nu(y) \le \left(~\int\limits_{\R^n}|f(x - y) - f(x)|^p\nu(y)\right)^{\frac{1}{p}} \cdot \underbrace{\left(~\int\limits_{\R^n}1^q\nu(y)\right)^{\frac{1}{q}}}_{1} = \\
            = \left(~\int\limits_{\R^n}|f(x - y) - f(x)|^p g(y)\d y\right)^{\frac{1}{p}} \underset{\text{\cref{p-equals-1}}}{\le} \left(~\int\limits_{\R^n}|f(x - y) - f(x)|\d y \cdot \int\limits_{\R^n}g(y)\d y\right)^{\frac1p}
            }
        }
    }
    ...
    \theorem{
        ...
        \provehere{
            \[(f * g)(x) = \int\limits_{\R^n}f(y)u(x - y)\d y \circlesign{=} \]
            ...
            Заметим, что для всякой последовательности $x_j \to x_0: u(x_j - y) \to u(x_0 - y)$, откуда при $x_j$, близких к $x_0$: $\exists C$: $|f(y)u(x_j - x_0)| \le C|f(y)|$, и можно применить теорему Лебега о мажоранте:
            \indentlemma{
    ...
    \definition[Аппроксимативная единица для $\R^n$]{
        Последовательность функций $g_j \in L^1(\R^n)$, $g_j \ge 0, \int\limits_{\R^n}g_j = 1, \forall \delta > 0: \int\limits_{|y| > \delta}g_j(y)\d y \underset{j \to \infty}\Map 0$
    }
    \theorem{
        Пусть $g_j$ --- аппроксимативная единица, $f \in L^p(\R^n)$, $f * g_j \underset{j \to \infty}{\overset{L^p}\Map} f$.

        Тогда $\|f * g_j - f\|_{L^p} \underset{j \to \infty}\Map 0$.

        Если $f$ непрерывна с компактным носителем (достаточно потребовать равномерной непрерывности), то $f * g_j \rightrightarrows f$
        \provehere{
            При $1 \le p < \infty$: \multline{\|(f * g_j) - f\|_{L^p} \le \int\limits_{\R^n}\int\limits_{\R^n}|f(x - y) - f(x)|^p g_j(y)\d x \d y = \int\limits_{\R^n}\int\limits_{\R^n}|f(x - y) - f(x)|^p\d x \cdot g_j(y)\d y =\\= \int\limits_{|y| < \delta}g_j(y)\int\limits_{\R^n}|f(x - y) - f(x)|^p\d x \d y + \int\limits_{|y| \ge \delta}g_j(y)\int\limits_{\R^n}\underbrace{|f(x - y) - f(x)|^p}_{\le 2^p\left(|f(x - y)|^p + |f(y)|^p\right)}\d x\d y}
            ...
            Для первого слагаемого применим непрерывность сдвигов для $f \in L^p(\R^n), 1 \le p < \infty$: $\forall \eps > 0: \exists \delta$: $|y| \le \delta \then \int\limits_{\R^n}|f(x - y) - f(x)|^p \d x < \eps$. Значит, оно тоже маленькое.
    ...
    Раз доказав это, мы получим, что так как левая часть почти всюду сходится к плотности $h$, и тогда окажется, что плотность как раз равна модулю якобиана.
    \lemma[Об искажении]{
    ...
    Но раз $\det \phi(x_0, \_) =E$, то по лемме об искажении и принципу двух полицейских ${\frac{|\phi(B_r(x_0))|}{|B_r(x_0)|} \underset{r \to 0}\Map 1}$:
