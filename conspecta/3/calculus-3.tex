...
%    Если же $\overline{\mu}(E) = \infty$
    ...
    Пусть $(S, \Sigma, \lambda)$ --- пространство с мерой $\lambda \ge 0$; предположим, что $\lambda$ --- $\sigma$-конечна. пусть $\nu$ --- вещественная мера на $\Sigma$.
    ...
    Пусть $\lambda_m(e) = 0$.
        ...
        По теореме Радона --- Никодима: $\exists w: U \map \R$ --- неотрицательная измеримая функция, такая, что \[\forall e \subset U: \rho(e) = \lambda_m(f(e)) = \int\limits_{e}w(x)\d x\label{density}\tag{$*$}\]
        \indent{
%            \theorem[Регулярность меры Лебега]{Рассмотрим евклидово пространство $\R^k$.
...
%                \provehere{
%                    Возьмём исчерпывающую последовательность компактов $K_1 \subset \Int K_2 \subset \Int K_3 \subset \cdots$
%                    Для всякого $j$ найдём суммируемую $g_j$, такую, что~\eqref{density} верна $\forall e \subset K_j$.
%
%                $\mu(e) = C_{(m)}\mu_m^*(F(e))$.
%
...
%            }
        ...
        \item Осталось показать, что $w(x) = |\det \Gamma(\d_f(x, \_))|^{\nicefrac12}$, а это делается с помощью слабой теоремы о дифференцируемости.
        Иными словами, покажем $\forall x \in U: \lim\limits_{r \to 0}\frac{\lambda_m(f(B_r(x)))}{|B_r(x)|} = |\det \Gamma(\d_f(x, \_))|^{\nicefrac12}$.
    ...
    \theorem[Хан]{
        Пусть $\mu$ --- \textbf{конечная} вещественная мера на $(X, \Sigma)$. Тогда $\exists E, F \in \Sigma: E \sqcup F = X$, такие, что $\mu\Big|_E \ge 0, \mu\Big|_F \le 0$.
        \provehere{
            Ранее доказано, что $\defset{\mu(b)}{b \in \Sigma}$ ограничено.
            Пусть $M = \sup\limits_{b \in \Sigma}\mu(b)$. Так как $\mu(\o) = 0$, то $M \ge 0$.
            \bullets{
