...
\begin{document}
    ...
    Пусть $\mathcal{A}$ --- полукольцо, лежащее в $\mathcal{C}$, $\mu$ --- счётно-аддитивная мера на $\mathcal{A}, \overline{\mu}$ --- стандартное продолжение меры (на $\mu$-измеримые множества из $\Sigma$).
...
%    Если же $\overline{\mu}(E) = \infty$
    ...
    \fact{\label{supp-size}Пусть $f \ge 0$ --- измеримая функция, обозначим $A = \defset{x}{f(x) \ne 0}$.
    ...
    Положим $h_k \coloneqq \sup\limits_{n \ge k}|f_n(x) - f(x)|$, заметим, что $h_k \searrow 0$.
        Так как $0 \le h_0(x) \le 2g(x)$, то $h_0$ суммируема, откуда по теореме Леви: $\int\limits_{X}h_k(x)\d\mu \underset{k \to \infty}\Map 0$.

    Осталось применить принцип двух полицейских для неравенства \[0 \le |f_k(x) - f(x)| \le h_k(x) \quad \then \quad \int\limits_{X}0\d\mu \le \int\limits_{X}|f_k(x) - f(x)|\d\mu \le \int\limits_{X}h_k(x)\d\mu\qedhere\]
    }
    }
    \counterexample{
    ...
    \provehere{
    Положим $h_k(x) = \inf\limits_{n \ge k}f_n(x)$. $h_k(x) \nearrow h(x) = \varliminf\limits_{n \to \infty}f_n(x)$. \[\int\limits_{X}h\d\mu = \lim\limits_{k \to \infty}\int\limits_{X}h_k\d\mu = \varliminf\limits_{k \to \infty}\int\limits_{X}h_k\d\mu \le \varliminf\limits_{n \to \infty}\int\limits_{X}f_n \d\mu\qedhere\]
    }
    }
    ...
    \fact{
    \down\numbers{
    \item Из сходимости $f_n \almost{n \to \infty} f$ следует сходимость по мере $f_n \underset{n \to \infty}{\overset{\mu}\Map} f$.
    \item Если $\mu(X) < \infty$, то из сходимости по мере $f_n \underset{n \to \infty}{\overset{\mu}\Map} f$ следует, что найдётся сходящаяся подпоследовательность: $\exists n_1 < n_2 < \dots$: $f_{n_k} \almost{k \to \infty} f$.
    }
    \provehere{\down\numbers{
    \item Пусть $\eps > 0$, обозначим $A_n \coloneqq \defset{x \in X}{\exists k \ge n: |f_k(x) - f(x)| \ge \eps}$. Они вложены: ${A_1 \supset A_2 \supset \dots}$
        ...
        Отметим, что $A_n$ измеримы, и пусть $A \coloneqq \bigcap\limits_{n = 1}^{\infty}A_n$. На множестве $A$ нет сходимости: $\begin{tikzcd}[ampersand replacement=\&,column sep=small,cramped]f_n \arrow[r, "/"{marking, allow upside down}] \& f\end{tikzcd}$.
        Но раз есть сходимость почти всюду, то $\mu(A) = 0$, то есть $\mu(A_n) \underset{n \to \infty}\Map 0$.
    \item \up \definition[Последовательность Коши по мере]{
    ...
    \theorem{
        ...
        Иными словами $\|f\| = 0 \iff f = 0$ почти всюду~(\ref{supp-size}).
    \provehere{
        \indentlemma[Неравенство Гёльдера]{
            ...
            Деля $f \rightsquigarrow \frac{f}{\left(\int\limits_{X}|f|^p\right)^{\nicefrac{1}{p}}}$ и $g \rightsquigarrow \frac{g}{\left(\int\limits_{X}|g|^q\right)^{\nicefrac{1}{q}}}$, получаем $\|f\|_{L^p} = \|g\|_{L^q} = 1$.
    ...
    Теперь проверим неравенство треугольника $\|f + g\|_{L^p} \le \|f\|_{L^p}+ \|g\|_{L^p}$.
        Для интегралов оно носит название \emph{неравенства Минковского}.
    ...
    $L^p(\mu)$ --- полно.
    \provehere{
    Рассмотрим последовательность Коши $f_n$, и пусть $E_{k,l} \coloneqq \defset{x \in X}{\abs{f_k(x) - f_l(x)} > \delta}$.

    По определению последовательности Коши $\forall \eps > 0: \exists N \in \N: k, l \ge N \then \int\limits_{X}|f_k - f_l|^p < \eps^p$.

        Тогда $\mu E_{k,l} = \mu\defset{x \in X}{\abs{f_k(x) - f_l(x)}^p > \delta^p} \le \frac{\eps^p}{\delta^p}$. Значит, $f_n$ --- последовательность Коши по мере.
    ...
    Отсюда видно, что $\mu(X) = 1$, то $\|f\|_{L^s(\mu)} \le \|f\|_{L^p(\mu)} \cdot \mu(X)^{\frac{1}{sr'}}$, что особенно красиво при \emph{вероятностной мере} --- $\mu(X) = 1$.
    ...
    \fact{
        ...
        \[\|f * g - f\|_{L^p} \le \int\limits_{\R^n}\int\limits_{\R^n}|f(x - y) - f(x)|^p\d y\cdot|g(x)| \d x\]
        \provehere{
    ...
    \theorem{
        ...
        \provehere{
            ...
            \abs{(f * g_j)(x) - g(x) } \le \int\limits_{|y| \le \delta}|f(x - y) - f(x)|g_j(y)\d y + \int\limits_{|y| > \delta}\left(|f(x - y)| + |f(x)|\right)g_j(y)\d y \le \eps + 2K\int\limits_{|y| > \delta}g_y(y)\d y \underset{j \to \infty}\Map 0}
        }
    }
    \fact{
        ...
        \provehere{
            Построим специальную аппроксимативную единицу. \numbers{
                ...
                \item Положим $a \coloneqq\int\limits_{\R}\phi \d x$. Положим $\psi = \frac{\phi}{a}$. Это функция с единичным интегралом.
    ...
    Наконец,
    \[\int\limits_{|x| > \delta}\Phi_j(x)\d x = \int\limits_{|x| > \delta}j^n \Phi(j x)\d x = \int\limits_{|x| > j\delta}\Phi(j x)\d x \underset{j \to \infty}\Map 0\]
    ...
    Тогда для $g \in L^p(\R^n)$: $g * \phi_t \underset{t \to 0}{\overset{L^p}\Map} g$.
    ...
    \note{
        ...
        \provehere{
            $g$ не может быть отрицательной на множестве положительной меры, иначе мера этого множества была бы меньше нуля.
        }
    }
    ...
    $\exists$ измеримая на $G$ функция $g$, измеримая по Лебегу, такая, что $|F(e)| = \int\limits_{e}g(x)\d x$.
    }
    ...
    \intfact[Теорема о дифференцировании]{
        Пусть $B_r$ --- шар (можно куб) радиуса $r$ с центром в $0 \in \R^n$.
    ...
    Справа как раз стоит выражение, которое мы хотим показать, что стремится к $h(x)$. Сходимость в $L^1$ означает сходимость по мере, а раз имеется сходимость по мере, то имеется последовательность $r_n$, стремящаяся к нулю, такая что $h * \phi_{r_n} \underset{n \to \infty}\Map h$ почти всюду.
    }
    }
    Осталось доказать, что всюду \[\frac{|F(x + B_r)|}{|B_r|} \underset{r \to 0}\Map |J_F(x)|\label{jacobian}\tag{$*$}\]
    Раз доказав это, мы получим, что раз левая часть почти всюду сходится к плотности $h$, и тогда получится, что плотность как раз равна модулю якобиана.
    \lemma[Об искажении]{
    ...
    \provehere{
        ...
        Возьмём $\eps > 0, \delta > 0$, для каждого $k$ выберем $\eps$-покрытие $\{a_{k,j}\}_{k}$ множества $e_k$, такое, что $\sum\limits_{j}\left(\frac{\diam a_{k,j}}{2}\right)^p < \mu_p(e_k, \eps) + \frac{\delta}{2^k}$.
        ...
        Устремляя $\delta \to 0$, получаем $\mu_p(e, \eps) \le \sum\limits_{k}\mu_p(a_k, \eps)$.
    }
    }
    }
    ...
    \fact{
    Если $\dist(e_1, e_2) > 0$, то $\mu^*_p(e_1, e_2) = \mu_p^*(e_1) + \mu_p^*(e_2)$.
    \provehere{
    В определении $\mu_p(e, \eps)$ можно ограничиться $\eps$-покрытиями $\{a_j\}$, такими, что $\forall j:a_j \cap e \ne \o$.

        В силу счётной полуаддитивности $\mu_p^*(e_1 \cup e_2) \le \mu_p^*(e_1) + \mu_p^*(e_2)$.

        Пусть $\delta = \dist(e_1, e_2)$, рассмотрим $\eps < \delta$.
    Пусть $\{a_j\}$ --- $\eps$-покрытие объединения, такое, что $\forall j: a_j \cap (e_1 \cup e_2) \ne \o$. Тогда для каждого $j: a_j \cap e_1 = \o$ либо $a_j \cap e_2 = \o$. Стало быть \[\sum\limits_{j}\left(\frac{\diam (a_j)}{2}\right)^p = \sum\limits_{j: a_j \cap e_1 \ne \o}\left(\frac{\diam (a_j)}{2}\right)^p + \sum\limits_{j: a_j \cap e_2 \ne \o}\left(\frac{\diam (a_j)}{2}\right)^p \ge \mu_p(e_1, \eps) + \mu_p(e_2, \eps)\qedhere\]
    }
    }
    \theorem{
    Пусть $(X, d)$ --- метрическое пространство, пусть $\eta$ --- предмера на $X$, причём $\forall e_1, e_2 \subset X: \dist(e_1, e_2) > 0 \then \eta(e_1 \cup e_2) = \eta(e_1) + \eta(e_2)$. Тогда все замкнутые множества в $X$: $\eta$-измеримы.
    \provehere{
    ...
    Пусть $F_n = \defset{x \in X}{\dist(x, F) < \frac{1}{n}}$. Из замкнутости $F$: $\bigcap\limits_{n \ge 1}F_n = F$.
    Пусть $B_n = a \sm F_n$.

    Если $x \in a \cap F$, а $y \in B_n$, то разумеется $\dist(x, y) \ge \nicefrac1n$.
        Таким образом, заведомо $\eta(a) \ge \eta(a \cap F) + \eta(B_n)$.
        Так как $B_n$, возрастая, в объединении дают $a \sm F$, то хочется поверить, что $\lim\limits_{n \to \infty}\eta(B_n) = \eta(a \sm F)$.
    Запишем
    \[a \sm F = \bigcup\limits_{n = 1}^{\infty}B_n = B_n \sqcup\bigsqcup\limits_{j \ge n}(B_{j+1} \sm B_{j})\]
    Из счётной полуаддитивности $\eta(a \sm F) \le \eta(B_n) + \sum\limits_{j \ge n}\eta(B_{j+1}\sm B_j)$, но с другой стороны ${\eta(a \sm F)\ge \eta(B_n)}$.
    Значит, надо доказать, что $\sum\limits_{j \ge n}\eta(B_{j+1}\sm B_j)\underset{j \to \infty}\Map 0$. Но это просто значит, что ряд $\sum\limits_{j = 1}^{\infty}\eta(B_{j+1}\sm B_j)$ сходится.

    Разобьём ряд на два: $\sum\limits_{j = 2k}\eta(B_{j+1}\sm B_j) + \sum\limits_{j = 2k+1}\eta(B_{j+1}\sm B_j)$.
    Теперь все подряд идущие слагаемые отстоят друг от друга на положительное расстояние: $\dist(B_{2k+1}\sm B_{2k}, B_{2k-1}\sm B_{2k-2}) > 0$.

    Стало быть, $\forall n: \sum\limits_{j = 2k}^{j \le n}\eta(B_{j+1}\sm B_j) \le \eta\left(\bigcup\limits_{j = 1}^{n}B_{j}\right) \le \eta(a)$.
    Аналогично сходится ряд ${\sum\limits_{j = 2k+1}\eta(B_{j+1}\sm B_j)}$.
    }
    }
    \corollary{
    ...
    \proposal{
        ...
        \provehere{
            ...
            Таким образом, $\mu_p(\Phi(a), C\eps) \le \mu_p(a, \eps)$.
        }
    }
    \corollary{
    ...
    \corollary{
    Пусть $\alpha = \inf\defset{p > 0}{\mu_*^p(a) < \infty}$. Тогда $\mu_s^*(a) = \infty$ при $s > \alpha$, и при $s < \alpha$, то $\mu^*_s(a) = +\infty$.
    }
    ...
    \provehere{
        ...
        Докажем, что $\mu_n^*(K) \in (0, \infty)$ (или можно сказать, что $\mu_n(K) \in (0, \infty)$, так как куб --- борелевское множество).
    ...
    Пусть $\{e_j\}$ --- $\eps$-покрытие куба $K$, и пусть $b_j$ --- шар радиуса $\eps$, содержащий $e_j$.
        \[1 = |K| \le \underset{\text{мера Лебега шара пропорциональна его радиусу}}{\le} c\sum\limits_{j}\left(\diam(e_j)\right)^n \le 4^n c\cdot \sum\limits_{j}\left(\frac{\diam(b_j)}2\right)^n \]
    В другую сторону пойдём так: разобьём куб $K$ на диадические кубы ранга $s$, пусть $K_s$ --- диадические кубы ранга $s$, содержащиеся в $K$.
    Тогда $\left\{\overline{K}_s\right\}_s$ образуют $\eps$-покрытие $K$ при $\eps = 2^{-s}\sqrt{n}$.
    Отсюда получаем оценку
    \[\mu_n^*\left(K, 2^{-s}\sqrt{n}\right) \le \sum\limits_{s}\left(\frac{\diam K_s}2\right)^n = 2^{sn} \cdot \left(\frac{2^{-s}\sqrt{n}}2\right)^n = \left(\frac{\sqrt{n}}{2}\right)^n\qedhere\]
    }
    }
    ...
    \fact{
    $\exists C_{(m)}: C^{\lambda}_{\mu}\mu_n = \lambda_n$.
    \provehere{
...
%    Если же ситуация не такова, то можно взять исчерпывающую последовательность компактов.
        \indent{
            \theorem[Регулярность меры Лебега]{
                $\forall$ измеримого по Лебегу $e \subset \R^k$, $\forall \eps > 0: \exists U \supset e: \abs{U \sm e} < \eps$.
                Кроме того, $|e| = \sup\limits_{K \subset e}|K|$, где $K$ --- компактны (запись другая, так как мера любого компактного множества конечна).
                \provebullets{
                    \item Пусть $|e| < \infty$.
                    Так как оно измеримо, то $\exists$ параллелепипеды $\{P_j\}_j$, такие, что $\bigcup\limits_{j}P_j \supset e, \abs{\bigcup\limits_{j}P_j} < |e| + \eps$.
                    Можно считать, что они открыты: параллелепипед с номером $j$ можно раздуть так, что $\tilde{P}_j \supset P_j$ открыт, и $\abs{\tilde{P}}_j < \frac{\eps}{2^j}$.
                    ...
                    \item Из предыдущего пункта можно найти подходящее замкнутое множество: пусть $a \coloneqq \R^k \sm b$, найдётся открытое $U \supset a, |U \sm a| < \eps$, тогда для замкнутого $F \coloneqq \R^k \sm U: |b \sm F| = |U \sm a| < \eps$.
    ...
    Пусть $\lambda_m(e) = 0$.
        Тогда $\mu_m(e) = 0$, и так как $f$ --- локально липшицево, то $\lambda_m(f(e)) = 0$.
    }
%            $\rho(e) = C_{(m)}\mu_m^*(F(e))$.
...
%        Тогда из локальной липшицевости $\mu_m^*(F(e)) = 0$.
    ...
    \item Пусть $m = 1$.
        ...
        Тогда $J_{\gamma}(x_0, \_) = \vect{\gamma_1'(x_0) \\ \vdots \\ \gamma_n'(x_0)}$, и $\abs{\Gamma(\d_{\gamma}(x_0, \_))} = \sqrt{|\gamma_1^2(x_0) + \cdots + \gamma_n^2(x_0)|}$.
    ...
    \item Пусть $G \subset \R^2$, и отображение представляет собой график: $F(x, y) = (x, y, \phi(x, y))$, где $\phi: \R \times \R \map \R$.
        Здесь матрица Якоби $J_F(x, y) = \vect{1  & 0 \\ 0 & 1 \\ \partial_1\phi(x, y) & \partial_2\phi(x, y)}$, здесь $\partial_i$ --- производная по $i$-му аргументу.
    ...
    \section{Элементы общей теории меры}
    Пусть $(X, \Sigma)$ --- пространство с $\sigma$-алгеброй, на которой мы будем рассматривать разные меры.

    Так как у этих мер могут быть разные множества меры нуль, то может не получиться пополнить одну, продолжив на новые подмножества --- элементы $\Sigma$-алгебры --- и другую, поэтому здесь никаких предположений о полноте мер скорее не будет.
    ...
    Множество значений любой комплексной меры ограничено.
    \provehere{
    ...
    В теореме Хана ниже, по-видимому, используется именно это, последнее, определение.
    \theorem[Хан]{
        ...
        \provehere{
            $\defset{\mu(b)}{b \in \Sigma}$ ограничено~(\ref{bounded}), пусть $M = \sup\limits_{b \in \Sigma}\mu(b)$. Так как $\mu(\o) = 0$, то $M \ge 0$.
            \bullets{
                \item $M = 0$. Тогда $\mu \le 0$, и $F = X, E = \o$.
                ...
                Теперь заметим, что $\overline{b_1} \supset \overline{b_2} \subset \cdots$ Положим $E \coloneqq \bigcup\limits_{k = 1}^{\infty}b_k$ В силу монотонной непрерывности $\mu(E) = \lim\limits_{k \to \infty}\mu(b_k) = M$.

                Теперь в силу леммы все подмножества $E$ имеют положительную меру. Положим $F \coloneqq X \sm E$, все подмножества $F$ имеют отрицательную меру (от противного: если $f \subset F$ имеет положительную меру, то $\mu(E \cup f) > M$).\qedhere
            }
        }
    }
    Такие $E, F \subset X$ из теоремы --- \emph{разложение Хана}.
    Оно единственно с точностью до множества меру нуль --- если $A \subset E$ имеет меру нуль, то все его подмножества тоже имеют меру нуль, и $(E \sm A, F \cup A)$ --- тоже разложение Хана.
    ...
    \note{
        ...
        \provehere{
            \[\int\limits_{e}g\d|\mu| = \int\limits_{e \cap E}\d|\mu| - \int\limits_{e \cap F}\d|\mu| = \int\limits_{e \cap E}\d\mu^+ - \int\limits_{e \cap F}\d\mu^- = \mu^+(e \cap E) - \mu^-(e \cap F) = \mu(e)\qedhere\]
        }
    }
    ...
    \note{
        ...
        \provehere{
            Если $a \in \Sigma$, и $e_1 \sqcup \cdots \sqcup e_n = a$, то $\sum\limits_{j =1}^{n}|\mu(e_j)| \le \sum\limits_{j = 1}^{n}\mu^+(e_j) + \mu^-(e_j) \le \mu^+(a) + \mu^-(a)$.
    ...
    \corollary{
        ...
        \provehere{Разложим $\rho = \rho_1 + i\rho_2$.}
    }
    \proposal{
        ...
        \provenumbers{
            ...
            \item Пусть $g = \sum\limits_{j = 1}^{k}c_j \chi_{e_j}$ --- простая функция, где мы считаем, что $b_j \in \Sigma$ попарно не пересекаются.
            ...
            \[\all{|\mu|(a) \le |\mu_1(a)|(a) + |\mu_2|(a) \\ |\mu_1|(a) \le |\mu|(a) + |\mu_2|(a) \qquad \then \quad |\mu_1|(a) - |\mu_2|(a) \le |\mu|(a) \le |\mu_1|(a) + |\mu_2|(a)}\]
            Оценим $\mu_2(a)$:
    ...
    \theorem[Лебег]{
        ...
        \provehere{
            Пусть $A = \sup\defset{|\rho|(e)}{e \in \Sigma, \lambda(e) = 0}$. Тогда $\exists e_n \in \Sigma: |\rho|(e_n) > A - \frac{1}{n}$.

            Положим $\overline{e_n} = e_n \cup e_{n + 1} \cup \cdots$. Тогда $\overline{e_1} \subset \overline{e_2} \subset \cdots$, и положим $E = \bigcup\limits_{n = 1}^{\infty}\overline{e_n}$.

            В силу монотонной непрерывности $|\rho|(E) = \lim\limits_{n \to \infty}|\rho|(e_n) = A$. Так как $\forall n: \lambda(\overline{e_n}) = 0$, то $\lambda(E) = 0$.

            Разложим $\rho_1(e) \coloneqq \rho(e \sm E), \rho_2(e) \coloneqq \rho(a \cap E)$. В таком представлении $\rho_2$ очевидно сингулярна относительно $\lambda$. Абсолютную непрерывность $\rho_1$ относительно $\lambda$ докажем от противного.
    ...
    \example{
        Определим рекурсивно канторову лестницу $[0, 1] \map [0, 1]$.
%        \[\begin{tikzpicture}
%              \draw[->] (-1,0) -- (3,0) node[right] {$x$};
%              \draw[->] (0,-1) -- (0,3) node[above] {$y$};
%              \newcommand{\depictCantor}[3]{
%                  \pgfmathsetmacro{\l}{(#1 + #1 + #2) / 3};
%                  \pgfmathsetmacro{\r}{(#1 + #2 + #2) / 3};
%                  \draw (\l,{(\l+\r)*0.5}) -- (\r,{(\l+\r)*0.5});
%                  \ifnum #3 < 5
%                      \depictCantor{#1}{\l}{\numexpr#3 + 1};
%                      \depictCantor{\r}{#2}{\numexpr#3 + 1};
%                  \fi
...
%              \depictCantor{0}{3}{0};
%        \end{tikzpicture}\]
        Построив по данной функции меру, мы получим меру, сосредоточенную на канторовом множестве меры нуль.
